...
Скрипт `tools/clang-format-all.sh`:

* обходит `src/include/app/tests` через `find`;
* запускает clang-format и показывает только реально изменённые файлы (каждый с иконкой 🟡), либо пишет, что всё в порядке;
* удобен на машинах без Meson.
